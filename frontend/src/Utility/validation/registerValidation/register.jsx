const messages = {
    ...
    NameError: () =>
      `frontend: your name must be between 3-20 character without any spacial characters and Spaces`,
      email: (val) =>
      `frontend: your email : ${val}  is not correct please enter avalid mail`,
      password: () =>
      `frontend: Please enter a Strong Password,
      8 to 24 characters. Must include uppercase ,lowercase,numbers at least one` ,
       confirm_password: () =>
  ...
  const rules = {
    ...
    username: (val) =>{
        const Name = /^[A-z 0-9]{3,20}$/;
        return(

            Name.test(val) ? "pass" :messages.NameError()
